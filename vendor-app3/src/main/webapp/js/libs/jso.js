(function(exp, $) {
	var config = {}, default_lifetime = 3600;
	/*
	 * ------ SECTION: Utilities
	 */
	/*
	...
	/*
	 * Takes an URL as input and a params object. Each property in the params is
	...
	/*
	 * Returns epoch, seconds since 1970. Used for calculation of expire times.
	 */
	var epoch = function() {
	 ...
	 */
	/*
	 * saveState stores an object with an Identifier. TODO: Ensure that both
	 * localstorage and JSON encoding has fallbacks for ancient browsers. In the
	 * state object, we put the request object, plus these parameters:
	 * restoreHash providerID scopes
	 */
	...
	var filterTokens = function(tokens, scopes) {
		...
		for (i = 0; i < tokens.length; i++) {
			usethis = true;
			// Filter out expired tokens. Tokens that is expired in 1 second
			// from now.
	...
	/*
	 * saveTokens() stores a list of tokens for a provider. Usually the tokens
	 * stored are a plain Access token plus: expires : time that the token
	 * expires providerID: the provider of the access token? scopes: an array
	 * with the scopes (not string)
	 */
	...
	/*
	 * Save a single token for a provider. This also cleans up expired tokens
	...
	/*
	 * Get a token if exists for a provider with a set of scopes. The scopes
	 ...
	 */
	/**
	 * Check if the hash contains an access token. And if it do, extract the
	 * state, compare with config, and store the access token for later use.
	 */
	var jso_checkfortoken = function() {
		var atoken, h = window.location.hash, now = epoch(), state, co;
		/*
		...
		/*
		 * Decide when this token should expire. Priority fallback: 1. Access
		 * token expires_in 2. Life time in config (may be false = permanent...)
		 * 3. Specific permanent scope. 4. Default library lifetime:
		 */
	...
	var jso_authrequest = function(providerid, scopes) {
		...
		// console.log("About to send an authorization request to [" +
		// providerid + "]. Config:")
		// console.log(co);
		state = uuid();
		request = {
		...
		// With the implciit grant flow, the hash will be replaced with the
		// access
	...
	$.oajax = function(settings) {
		var allowia, scopes, token, providerid, co;
